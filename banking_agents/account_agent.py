...
from tools.account_tools import get_account_balance, get_transaction_history, transfer_funds, verify_login
...
5. **Login Verification:** Verify account credentials when a customer asks to log in or authenticate.
6. **Tone:** Professional, concise, and reassuring. Always confirm actions taken.

You have access to: get_account_balance, get_transaction_history, transfer_funds, verify_login.
Always confirm the action completed and summarize results for the customer.
""",
    tools=[get_account_balance, get_transaction_history, transfer_funds, verify_login],
)